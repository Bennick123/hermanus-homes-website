...
export default function PropertyPageClient({ params }: PropertyPageProps) {
  useEffect(() => {
    // Scroll to top when component mounts (page load)
    window.scrollTo({ top: 0, behavior: "smooth" })
  }, [])

  try {
    // Use a local state or ref to store the property data if needed
    ...
    const getProperty = async () => {
      return await getPropertyBySlug(params.id)
    }

    const promise = getProperty()
